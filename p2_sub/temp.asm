  ...
  .text
    ...
    new_line:      .asciiz   "\n"
          ...
          .finish_up: addi $t3, $t3, 1 #i++
                      ...
                      addi $sp, $sp, -4 #shift stack pointer down to save $ra
                      sw $ra, ($sp) #save ra
                      move $a3, $a0 #save bigint a in t0
                      move $a0, $a2 #move bigint c into a
                      jal compress #call compress on (a).
                      move $a0, $a3 #reset a
                      lw $ra, 0($sp) #reload ra
                      addi $sp, $sp, 4 #move stack pointer back up
                      jr $ra
          ...
          pow_big: #a0 has a, #a1 has the result final result holder, #a2 has p
                          .text
                          addi $sp, $sp, -24 #shift stack pointer down to save $ra and $s0
                          sw $s4, 20($sp)
                          sw $s3, 16($sp)
                          sw $s2, 12($sp)
                          sw $s1, 8($sp)
                          ...
                          move $s4, $a0 #save the a0 reference
                          move $t0, $0 #i = 0
                          lw $t4, ($a0) #grab a.n
             ...
             .set_b_loop: sll $t5, $t0, 2 #byte offset of this current i
                  ...
                  .loop7: jal mult_big #call mult big with (a, b, c)
                          addi $s4 $s4, 1 #i++
                          #move $a0, $s4 #set a as the first parameter
                          ...
                          #need to copy over value in $a2 to c and reset stack
                          move $t0, $0 #i = 0
                          lw $t4, ($s2) #grab res.n
                          sw $t4, ($s3) #set c.n = res.n
             .set_c_loop: sll $t5, $t0, 2 #byte offset of this current i
                          add $t7, $t5, $s2 #calculate address for res.digits[i] - 4
                          ...
                          lw $s0, 4($sp)
                          lw $ra, ($sp)
                          addi $sp, $sp, 24 #shift stack pointer back up
                          jr $ra
